...
export const SovereignLoginGate: React.FC = () => {
  ...
  const handleGoogleSignIn = async (roleToUse?: UserRole) => {
    setError('');
    setIsSubmitting(true);
    // Make sure role is selected for Google login; if not selected, default to citizen
    const targetRole = roleToUse || 'citizen';
    try {
          ...
          <div className="p-6 sm:p-7 space-y-4">
            ...
            {activeTab === 'signin' && (
              <form onSubmit={handleSignIn} className="space-y-4">
                ...
                <button
                  type="button"
                  onClick={() => handleGoogleSignIn(selectedRole || 'citizen')}
                  disabled={isSubmitting}
                ...
                >
                  ...
                  <span>
                    Sign In with Google ({selectedRole ? selectedRole.replace('_', ' ').toUpperCase() : 'CITIZEN'})
                  </span>
                </button>
              </form>
            )}
            ...
            {activeTab === 'register' && (
              <form onSubmit={handleRegister} className="space-y-3">
                ...
                <div className="relative flex items-center py-2.5">
                  <div className="flex-grow border-t border-[#ded7ca]" />
                  <span className="mx-3 text-[10px] font-editorial-mono uppercase font-bold text-[#7b817c]">
                    or register &amp; sign in with google
                  </span>
                  <div className="flex-grow border-t border-[#ded7ca]" />
                </div>

                {/* Google Sign In based on role in registration */}
                <div className="space-y-1.5">
                  <button
                    type="button"
                    onClick={() => handleGoogleSignIn(regRole || 'citizen')}
                    disabled={isSubmitting}
                    className="w-full py-2.5 rounded-xl border border-[#ded7ca] bg-white text-xs font-bold text-[#263a42] flex items-center justify-center gap-2.5 hover:bg-[#fbfaf6] shadow-2xs transition disabled:opacity-50 cursor-pointer"
                  >
                    <svg className="w-4 h-4" viewBox="0 0 24 24">
                      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                    ...
                    <span>
                      Sign In with Google as {
                        regRole === 'contractor'
                          ...
                          : regRole === 'material_contractor'
                          ? 'Material Vendor'
                          : 'Citizen Auditor'
                      }
                    </span>
                  </button>
                  <p className="text-[10px] text-center text-[#7b817c]">
                    Role for Google sign-in: <strong className="text-[#263a42] uppercase">{regRole ? regRole : 'citizen (default)'}</strong>
                  </p>
                </div>
              </form>
            )}
