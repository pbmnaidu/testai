...
import { commitSyncDiff, fetchSyncHealth, fetchSyncHistory, fetchSyncStatus, fetchTrainingStatus, previewSyncDiff, startSync, startTraining } from '../services/api';
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {status?.job && status.job.status !== 'IDLE' && <div className="bg-white rounded-2xl border border-indigo-200 p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between gap-4"><div><h3 className="text-sm font-bold text-slate-900 flex items-center gap-2"><Loader2 className={status.job.status === 'RUNNING' || status.job.status === 'QUEUED' ? 'w-4 h-4 text-indigo-600 animate-spin' : 'w-4 h-4 text-indigo-600'} />Dataset synchronization</h3><p className="text-xs text-slate-500 mt-1">{status.job.message || 'Synchronization status unavailable.'}</p></div><span className="text-xs font-mono font-bold text-indigo-700">{status.job.status}</span></div>
