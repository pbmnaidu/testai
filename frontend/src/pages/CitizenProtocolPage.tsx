...
import { fetchCitizenEvidenceStats, fetchCitizenWorks, fetchFilters } from '../services/api';
import { CitizenEvidenceConfig, CitizenEvidenceRecord, PublicWorkRecord, FilterOptions } from '../types';
...
export const CitizenProtocolPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    setError('');
    const params: Parameters<typeof fetchCitizenWorks>[0] = {
      limit: 250,
      state: state !== 'ALL' ? state : undefined,
      constituency: constituency !== 'ALL' ? constituency : undefined,
      status,
      category,
    ...
    return () => { active = false; };
  }, [state, constituency, status, category, search, nearby, refreshToken]);
