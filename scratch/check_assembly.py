...
def get_work_detail_by_path(work_id: str):
    return _fetch_work_detail_internal(work_id)
        ...
        "work_category": text("work_category", "Unclassified"),
        "state": text("state"),
        "constituency": text("constituency"),
        "mp_name": text("mp_name") or None,
        "work_status": text("work_status", "Unknown"),
        "sanction_amount": sanction,
        "effective_expenditure": expenditure,
        "recommended_date": date_value("recommended_date"),
        "sanction_date": date_value("sanction_date"),
        "completion_date": date_value("completion_date"),
        "estimated_completion_date": date_value("estimated_completion_date"),
        "progress_pct": progress,
        "latitude": location.get("latitude"),
        "longitude": location.get("longitude"),
        "coordinate_source": location.get("coordinate_source"),
        "coordinate_available": location.get("latitude") is not None and location.get("longitude") is not None,
        "normalized_status": _normalized_citizen_work_status(text("work_status", "Unknown")),
...
def _citizen_work_records() -> tuple[list[dict], dict]:
    data = get_data()
    coordinates = _load_published_work_coordinates(data)
    evidence = _read_citizen_evidence()
    evidence_counts = {}
    for record in evidence:
        key = str(record.get("work_id") or "").strip()
        evidence_counts[key] = evidence_counts.get(key, 0) + 1
    base_records = data.get("citizen_public_base_records")
    if base_records is None:
        frame = _all_records_frame(data).copy()
        if frame.empty:
            return [], {}
        # Avoid converting the analytical diagnostics columns (which include
        # nested arrays) for a public browsing endpoint. Only the public card
        # fields are needed here.
        public_columns = [
            "work_id", "description", "work_category", "state", "constituency", "mp_name",
            "work_status", "sanction_amount", "effective_expenditure", "recommended_date",
            "sanction_date", "completion_date", "estimated_completion_date", "expenditure_progress_pct",
        ]
        frame = frame[[column for column in public_columns if column in frame.columns]].copy()
        positions = {column: index for index, column in enumerate(frame.columns)}
        base_records = [_citizen_public_work_fast(row, positions, coordinates) for row in frame.itertuples(index=False, name=None)]
        data["citizen_public_base_records"] = base_records
    records = [dict(record, citizen_evidence_count=int(evidence_counts.get(record["work_id"], 0))) for record in base_records]
    # Put works with a real published position first so the first responsive
    # page can populate the map without loading thousands of unlocated rows.
    records.sort(key=lambda record: (not record["coordinate_available"], record["work_id"]))
...
def _haversine_distance_meters(latitude_a: float, longitude_a: float, latitude_b: float, longitude_b: float) -> float:
    ...
    haversine = math.sin(delta_lat / 2) ** 2 + math.cos(lat_a) * math.cos(lat_b) * math.sin(delta_lng / 2) ** 2
...
def _require_citizen_officer(request: Request) -> str:
    ...
    if CITIZEN_ALLOW_LOCAL_OFFICER_REVIEW and request.headers.get("x-mplads-role", "").casefold() == "officer":
        return "local_officer"
    raise HTTPException(status_code=403, detail="Officer authorization is required to update evidence review status.")
...
def get_citizen_works(
        ...
        "stats": {
            ...
            "works_with_coordinates": sum(1 for record in all_public_records if record["coordinate_available"]),
    ...
    if latitude is not None or longitude is not None:
        ...
        for record in records:
            if record["latitude"] is None or record["longitude"] is None:
                continue
            record = dict(record)
            record["distance_from_you_meters"] = _haversine_distance_meters(latitude, longitude, record["latitude"], record["longitude"])
            if record["distance_from_you_meters"] <= radius:
                nearby.append(record)
        records = sorted(nearby, key=lambda record: record["distance_from_you_meters"])

    total = len(records)
    start, end = (page - 1) * limit, page * limit
    normalized_statuses = [record["normalized_status"] for record in all_public_records]
    available_categories = sorted({record["work_category"] for record in all_public_records if record.get("work_category")})
    return {
        "total": total,
        "page": page,
        "limit": limit,
...
def get_citizen_evidence_stats():
    ...
    for record in records:
        ...
        try:
    return {"total": total, "page": page, "limit": limit, "total_pages": int(math.ceil(total / limit)) if total else 0, "records": records[start:end]}
...
async def create_citizen_evidence(
    ...
    if parsed_longitude is not None and not -180 <= parsed_longitude <= 180:
        ...
        records.sort(key=lambda record: str(record.get("uploaded_at") or ""), reverse=True)
    total = len(records)
    start, end = (page - 1) * limit, page * limit
    return {"total": total, "page": page, "limit": limit, "total_pages": int(math.ceil(total / limit)) if total else 0, "records": records[start:end]}
...
def _attendance_location_status(latitude, longitude, gps_accuracy, work: dict) -> tuple[str, float | None]:
    # Attendance requires a live device fix. The comparison against the
    # published work coordinate remains a review signal, never an automatic
    # rejection of a capture.
    if latitude is None or longitude is None:
        return "LOCATION_NOT_AVAILABLE", None
    if gps_accuracy is not None and gps_accuracy > CITIZEN_GPS_ACCURACY_THRESHOLD_METERS:
        return "LOW_GPS_ACCURACY", None
...
def _attendance_work_lookup() -> dict[str, dict]:
    records, _ = _citizen_work_records()
    return {record["work_id"]: record for record in records}
    if len(content) > ATTENDANCE_MAX_UPLOAD_BYTES:
        raise HTTPException(status_code=413, detail="The attendance image is larger than the configured limit.")
    image_type = _detect_image_type(content)
    if image_type not in {"jpg", "png", "webp"}:
        raise HTTPException(status_code=415, detail="Only a valid camera JPEG, PNG, or WebP image is accepted.")

    now = datetime.now().astimezone().isoformat()
    attendance_id = f"ATT-{datetime.now().strftime('%Y%m%d')}-{uuid.uuid4().hex[:10].upper()}"
    image_hash = hashlib.sha256(content).hexdigest()
    filename = f"{attendance_id}.{image_type}"
    image_path = os.path.join(ATTENDANCE_MEDIA_DIR, filename)
    location_status, distance = _attendance_location_status(latitude, longitude, gps_accuracy, work)
    with _CITIZEN_STORE_LOCK:
        existing = _read_attendance_records()
        duplicate_flag = any(record.get("work_id") == clean_work_id and record.get("image_sha256") == image_hash for record in existing)
        ...
        record = {
            ...
            "image_reference": f"/api/attendance/media/{filename}",
            ...
            "review_comment": None,
            "duplicate_flag": duplicate_flag,
            "integrity_note": "Camera-only UI assertion with server receipt, GPS metadata, and SHA-256 image hash; officer verification remains required.",
            "audit_events": [{"event": "created", "at": now, "status": "SUBMITTED", "source": "LIVE_CAMERA"}],
            "created_at": now,
            "updated_at": now,
        }
        existing.append(record)
        _write_attendance_records(existing)
    return record
